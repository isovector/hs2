...
instance {-# INCOHERENT #-} GBimap a b c d (K1 _1 a) (K1 _2 b) where
  gbimap f _ (K1 a) = K1 $ f a

instance {-# INCOHERENT #-} GBimap a b c d (K1 _1 c) (K1 _2 d) where
  gbimap _ g (K1 b) = K1 $ g b

instance {-# INCOHERENT #-} Bifunctor p => GBimap a b c d (K1 _1 (Loc c (p a c))) (K1 _2 (Loc d (p b d))) where
  gbimap f g (K1 loc) = K1 $ bimap g (bimap f g) loc

instance {-# INCOHERENT #-} Bifunctor p => GBimap a b c d (K1 _1 (p a c)) (K1 _2 (p b d)) where
  gbimap f g (K1 loc) = K1 $ bimap f g loc

instance {-# INCOHERENT #-} GBimap a b c d (K1 _1 x) (K1 _2 x) where
  gbimap _ _ (K1 x) = K1 x

instance (Functor q, GBimap a b c d (K1 _1 i) (K1 _2 o))
    => GBimap a b c d (K1 _1 (q i)) (K1 _2 (q o)) where
  gbimap f g (K1 loc) =
    K1 $ fmap (unK1 . gbimap @_ @_ @_ @_ @(K1 _1 i) @(K1 _2 o) f g . K1) loc
